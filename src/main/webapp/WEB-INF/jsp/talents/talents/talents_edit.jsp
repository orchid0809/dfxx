							...
							<tr>
								...
								<td>
									<label style="float:left;padding-left: 8px;padding-top:7px;">
										<input name="SEX" class="ace" id="form-field-radio1" type="radio" value="0"  <c:if test="${pd.SEX==0 or pd.SEX==null}">checked</c:if>>	<span class="lbl">男</span>

									</label>
									<label style="float:left;padding-left: 5px;padding-top:7px;">
										<input name="SEX" class="ace" id="form-field-radio2" type="radio" value="1" <c:if test="${pd.SEX==1}">checked</c:if>>	<span class="lbl">女</span>
								...
								<td>
									<div class="col-sm-9" style="width:30%;">
										<select class="chosen-select form-control" name="WORKING_TIME" id="id" data-placeholder="请选择">
											<option value="1年" <c:if test="${pd.WORKING_TIME == '1年'}">selected</c:if>>1年</option>
											<option value="2年" <c:if test="${pd.WORKING_TIME == '2年'}">selected</c:if>>2年</option>
											<option value="3年" <c:if test="${pd.WORKING_TIME == '3年'}">selected</c:if>>3年</option>
											<option value="4年" <c:if test="${pd.WORKING_TIME == '4年'}">selected</c:if>>4年</option>
											<option value="5年" <c:if test="${pd.WORKING_TIME == '5年'}">selected</c:if>>5年</option>
											<option value="6年" <c:if test="${pd.WORKING_TIME == '6年'}">selected</c:if>>6年</option>
											<option value="7年" <c:if test="${pd.WORKING_TIME == '7年'}">selected</c:if>>7年</option>
											<option value="8年" <c:if test="${pd.WORKING_TIME == '8年'}">selected</c:if>>8年</option>
											<option value="9年" <c:if test="${pd.WORKING_TIME == '9年'}">selected</c:if>>9年</option>
											<option value="10年" <c:if test="${pd.WORKING_TIME == '10年'}">selected</c:if>>10年</option>
											<option value="11年" <c:if test="${pd.WORKING_TIME == '11年'}">selected</c:if>>11年</option>
											<option value="12年" <c:if test="${pd.WORKING_TIME == '12年'}">selected</c:if>>12年</option>
											<option value="13年" <c:if test="${pd.WORKING_TIME == '13年'}">selected</c:if>>13年</option>
										</select>
									</div>
								</td>
							</tr>
