...
public class RecruitController extends BaseController {
	...
	public ModelAndView save(RecruitInfoParam param) throws Exception{
		...
		if(flag == null) {
			mv.setViewName("redirect:/recruit/list?flag=print");
	...
	public ModelAndView goEdit()throws Exception{
		...
		mv.setViewName("recruit/recruit/recruit_edit");
		mv.addObject("msg", "edit");
		//String recruitId = pd.get("RECRUIT_ID")== null ? null:(String)pd.get("RECRUIT_ID");
	...
	@ResponseBody
	public Object deleteRecruitInfo() throws Exception {
		PageData pd = new PageData();
		Map<String,Object> map = new HashMap<String,Object>();
		pd = this.getPageData();

		this.recruitInfoManager.delete(pd);
	...
	@RequestMapping("/recruit_print")
	public ModelAndView rental_print(Page page)throws Exception{
		...
		String select = pd.getString("isSelect");
		if("1".equals(select)){
			pd.put("searchData", DateUtil.fomatDateToString(new Date()));
		}

		page.setPd(pd);
		page.setShowCount(Integer.parseInt(pd.getString("record_cnt")));
		List<PageData>	varList = recruitService.list(page);	//列出Recruit列表
		List<PageData> temps = new ArrayList<PageData>();
		if(varList != null && varList.size() > 0){
			for(PageData pageData : varList){
		...
		varList.removeAll(temps);
		mv.setViewName("recruit/recruit/recruit_print_list");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
	}
